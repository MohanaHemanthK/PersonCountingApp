...
class StrongSORT(object):
    ...
    def _xywh_to_tlwh(bbox_xywh):
        ...
        if bbox_tlwh.size == 0:
            # If bbox_tlwh is empty, return an empty array
            return bbox_tlwh

        if len(bbox_tlwh.shape) == 1:
            # If bbox_tlwh is 1-dimensional, use single indexing
            bbox_tlwh[0] = bbox_tlwh[0] - bbox_tlwh[2] / 2.
            bbox_tlwh[1] = bbox_tlwh[1] - bbox_tlwh[3] / 2.
        else:
            # If bbox_tlwh is 2-dimensional, use double indexing
            bbox_tlwh[:, 0] = bbox_tlwh[:, 0] - bbox_tlwh[:, 2] / 2.
            bbox_tlwh[:, 1] = bbox_tlwh[:, 1] - bbox_tlwh[:, 3] / 2.
